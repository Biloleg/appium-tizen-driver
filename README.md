...
```bash
appium driver install --source=npm appium-tizen-tv-driver@github:Biloleg/appium-tizen-driver
```
...
| `appium:chromedriverExecutable` | _[Optional]_ Path to a specific Chromedriver executable. Use this if you want to manually specify the Chromedriver binary when `autodownloadEnabled` is `false`. For most Tizen TVs, Chromedriver 2.36 works best. When specified, this takes priority over auto-detection. |
| `appium:chromedriverExecutableDir` | _[Optional; default `~/.appium/chromedrivers`]_ Full path to the folder where chromedriver executables are located and will be automatically downloaded. The driver automatically downloads the correct Chromedriver version for your TV's Chrome version. If the Chrome version on the TV is lower than v58, the driver will use `Chrome/58.0.3029.0` to ensure compatibility with chromedriver 2.31. |
| `appium:autodownloadEnabled` | _[Optional; default `true`]_ Enable or disable automatic Chromedriver download. Set to `false` if you want to manually manage Chromedriver using `chromedriverExecutable` or place it manually in `chromedriverExecutableDir`. When enabled, the driver will automatically download the matching Chromedriver version for your TV's Chrome version. |
...
| `appium:app` | An absolute path to your `.wgt` app file, if you want Appium to install the app. The app file name must not include any characters that need to be escaped. `tizen` command/platform fails to install such files even with escapes. e.g. spaces such as `"/path/to/app file.wgt"`. It could also be an URL to a remote location.|
| `appium:appPackage`| The app package ID, if you want Appium to use an app already on the TV.|
| `appium:rcMode`| One of `js`, `remote`, or `api`. If `js`, the driver will use Chromedriver to mimic remote control keypresses. If `remote`, the driver will use a websocket-based input device API. If `api`, the driver will use Tizen TV Input Device API (`tizen.tvinputdevice`). See the `appium:rcToken` capability below for `remote` mode. Defaults to `js`.|
...
No additional configuration needed! The driver will:
...
2. Option A - Specify exact Chromedriver path:
```json
{
  "appium:autodownloadEnabled": false,
...
3. Option B - Place Chromedriver in the directory:
   - Download the appropriate Chromedriver version (e.g., 2.36 for most TVs)
   - Place it in `chromedriverExecutableDir` (default: `~/.appium/chromedrivers`)
   - Make it executable: `chmod +x /path/to/chromedriver`
   - The driver will find and use it automatically
...
The keycodes should be the string taken from values of the [`Keys` object](../tizen-remote/lib/keys.js)
exported by the `tizen-remote` package.
