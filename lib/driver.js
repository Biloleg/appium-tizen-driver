...
class TizenTVDriver extends BaseDriver {
   ...
   */
  async getPageSource() {
    // First get the raw HTML from Chromedriver
    // @ts-ignore
    const rawSource = /** @type {string} */ (await this.#chromedriver.sendCommand('/source', 'GET'));

    // Inject attributes into all elements using JavaScript execution
    const script = `
    ...
    try {
      ...
      log.warn(`Failed to inject attributes into page source: ${error.message}`);
    }
  ...
  /**
   * Find an element - converts unsupported locator strategies
  ...
  /**
   * Find elements - converts unsupported locator strategies
   * @param {string} strategy - Locator strategy
   * @param {string} selector - Selector value
   * @returns {Promise<import('@appium/types').Element[]>}
   */
